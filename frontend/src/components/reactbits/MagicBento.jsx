...
const ParticleCard = ({
  ...
  }, [animateParticles, clearAllParticles, disableAnimations, enableTilt, enableMagnetism, clickEffect, glowColor, onCardActivate]);
...
const MagicBento = ({
  ...
  const items = cards?.length ? cards : cardData;
        ...
        <div className={`card-responsive grid gap-2 bento-layout-${layout}`}>
          {items.map((card, index) => {
            ...
            return (
              <div
                key={card.title || index}
                className={`${baseClassName}${onCardClick ? ' cursor-pointer' : ''}`}
                style={cardStyle}
